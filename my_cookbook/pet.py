...
class Pet():
    def __init__(self, name, hp, status, known_states,  image, imagelist, level=0, exp=0):
        self.name = name
        self.hp = 100
        self.known_states = {'hungry':hungry, 'normal':normal} #List with active states
        self.status = normal
        self.image = image
    ...
    def receive_food(self, food):
        ...
        if self.known_states['hungry'].value > 0:
            self.status = self.known_states['normal'] 
